...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write('[INFO] Training Random Forest model...')
        use_real = not options['synthetic_only']
        try:
            train_model(use_real_data=use_real)
            self.stdout.write(self.style.SUCCESS('[SUCCESS] Model trained and saved successfully!'))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'[ERROR] Training failed: {e}'))
